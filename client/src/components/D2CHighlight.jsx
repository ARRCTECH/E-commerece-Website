...
export default function D2CHighlight() {
  ...
  return (
    <section className="overflow-hidden bg-[#f5f1ea] text-neutral-900 font-sans">
      {/* Ambient warm glow */}
      ...
      <div className="mx-auto flex max-w-7xl flex-col px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-1 flex-col items-center justify-center py-12 text-center sm:py-16 lg:py-20">
          ...
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:gap-4">
            ...
            {/* Our Story Button - Navigates to /about */}
            <button
        ...
        {/* Process — responsive grid: 3 cols on all screens */}
        <div className="mt-auto grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
          {steps.map((step, i) => (
            <div
              key={step.title}
              className="group relative overflow-hidden rounded-xl border border-neutral-900/10 bg-white/70 p-4 backdrop-blur-sm transition-all hover:border-[#b8902c]/50 hover:bg-white hover:shadow-md sm:p-5"
            >
              <div className="flex items-center justify-between">
                <step.icon
                  className="h-5 w-5 text-[#b8902c] sm:h-6 sm:w-6"
                  strokeWidth={1.5}
                />
                <span className="font-serif text-xs italic text-neutral-400 sm:text-sm">
                  0{i + 1}
                </span>
              </div>
              <h3 className="mt-3 font-serif text-xl font-light text-neutral-900 sm:text-2xl lg:text-3xl">
                {step.title}
              </h3>
              <p className="mt-1 text-[10px] uppercase tracking-[0.2em] text-neutral-500 sm:text-xs">
                {step.desc}
              </p>
            </div>
          ))}
